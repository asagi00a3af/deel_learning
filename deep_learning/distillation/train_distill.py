...
class DistillClassifier(chainer.link.Chain):
    ...
    def __call__(self, x, t):
        ...
        self.loss = None
        self.acc = None
        self.y = self.student(x)
        # predict teacher logit
        with chainer.using_config('train', False):
            with chainer.using_config('enable_backprop', False):
                teacher_y = self.teacher.predictor(x)
        self.loss = F.mean_squared_error(self.y, teacher_y)
...
class WideResNet(chainer.Chain):
    ...
    def __init__(self, widerate=4, num_class=10, blocks=(3, 4, 6)):
        ...
        with self.init_scope():
            ...
            self.block2 = resnet.BuildingBlock(
                blocks[0], 16 * widerate, 16 * widerate, 32 * widerate, stride=2)
            self.block3 = resnet.BuildingBlock(
                blocks[1], 32 * widerate, 32 * widerate, 64 * widerate, stride=2)
            self.block4 = resnet.BuildingBlock(
                blocks[2], 64 * widerate, 64 * widerate, 128 * widerate, stride=2)
...
def main():
    ...
    parser.add_argument('--widerate', type=int, default=1,
                        help='WideResnet wide parameter')
    parser.add_argument('--blocks', nargs='+', type=int,
                        default=[1, 1, 1], help='WideResnet blocks parameter')
    args = parser.parse_args()

    # save didrectory
    outdir = path.join(FILE_PATH, 'results/cifar10_wideresnet_{}_{}_lr_{}_distillation'.format(
    ...
    # laod teacher model
    teacher = L.Classifier(WideResNet(num_class=10, widerate=4, blocks=[3, 4, 6]))
    if args.gpu >= 0:
        teacher.to_gpu()
    serializers.load_npz(path.join(FILE_PATH, 'models/cifar10_wideresnet_4_346.npz'), teacher)
    # prepare student model
    model = DistillClassifier(WideResNet(
        num_class=10, widerate=args.widerate, blocks=args.blocks), teacher)
    if args.gpu >= 0:
        model.to_gpu()
